...
async def get_workspace_index_stats():
    """Workspace indexer status (stub — ChromaDB disabled)."""
    return {"watcher_active": False, "pending_files": 0, "note": "Workspace watcher disabled (ChromaDB free)"}

@router.get("/{category}", response_model=List[MemoryEntry])
async def list_memories_by_category(category: MemoryCategory, limit: int = 100, offset: int = 0):
    """Get all memories in a specific category collection."""
    return await memory_service.list_memories(category=category, limit=limit, offset=offset)
