...
class ConnectedDevice:
    ...
    def is_alive(self) -> bool:
        # Tolerate 3 missed heartbeats before treating the device as dead.
        # Was a hardcoded 90s; now derived from the heartbeat interval.
        from backend.config import settings  # local import to avoid cycle
        threshold = max(90, 3 * settings.ws_heartbeat_interval)
    ...
    async def broadcast(
        ...
        for device_id, device in list(self._connections.items()):
            ...
            try:
                ...
                log.debug("broadcast_target_dropped", device_id=device_id, error=str(e))
    ...
    async def send_heartbeat_pings(self) -> None:
        ...
        for device_id, device in list(self._connections.items()):
            ...
            try:
                ...
                log.debug("heartbeat_ping_failed", device_id=device_id, error=str(e))
                disconnected.append(device_id)
