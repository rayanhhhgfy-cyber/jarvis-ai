...
class ScreenshotManager:
    ...
    async def capture_screenshot(self, filepath: str = "screenshot.png") -> bool:
        """Async wrapper: captures screen and saves to disk. Returns True on success."""
        try:
            data = self.capture()
            if data and len(data) > 100:
                out_path = self.output_dir / filepath
                out_path.write_bytes(data)
                return True
            return False
        except Exception as e:
            log.error("capture_screenshot_failed", error=str(e))
            return False
